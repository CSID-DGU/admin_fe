      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
        <div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 6 }}>
            <span style={{ fontSize: 32, fontWeight: 700, color: "var(--decs-text-heading)" }}>{server.gpuName}</span>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: 32, fontWeight: 700, color: "var(--decs-text-heading)" }}>{`${server.daysLeft}일`}</div>
        </div>
      </div>
...
function UserDashboard({ onRequest, onConnect, onExtend, onDetail, userName = DECS_USER.userName, server = DECS_USER.server, expiryDays = DECS_USER.expiryDays, activities = DECS_USER.activities }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--decs-space-l)", maxWidth: 900, margin: "0 auto" }}>
      <div>
        <h1 style={{ margin: 0, fontSize: "var(--decs-fs-display-l)", lineHeight: 1.1, fontWeight: 700, color: "var(--decs-text-heading)" }}>안녕하세요, {userName}님</h1>
